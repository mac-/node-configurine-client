...
module.exports = function ConfigurineClient(options) {
	...
	this.createNewClientByConfigClientId = function(configClientId, email, callback) {
		if (!configClientId || !email || !callback) {
			throw new Error('Missing parameters');
		}
		var postBody = {
	...
	this.getConfigClientByConfigClientId = function(configClientId, callback) {
		if (!configClientId || !callback) {
			throw new Error('Missing parameters');
		}
		...
		getToken(clientId, sharedKey, function(err, accessToken) {
			...
			var url = configurineHost + '/clients/' + configClientId;
	...
	this.updateConfigClientFlags = function(configClientId, email, isConfirmed, isAdmin, callback) {
		if (!configClientId || !email ||
		...
		getToken(clientId, sharedKey, function(error, accessToken) {
			if (error) {
	...
	this.deleteConfigClientByConfigClientId = function(configClientId, callback) {
		if (!configClientId || !callback) {
			throw new Error('Missing parameters');
		}
